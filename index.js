...
let totalFunds = GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0);
let unfundedText = `A total of $${totalFunds.toLocaleString()} has been raised for ${GAMES_JSON.length} game${GAMES_JSON.length !== 1 ? 's' : ''}. Currently, ${unfundedGames} game${unfundedGames !== 1 ? 's' : ''} are still unfunded. We need your help to fund these amazing games!`;
...
const firstGame = {...topGame};
const secondGame = {...runnerUp};
